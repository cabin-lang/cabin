...
static BUILTINS: phf::Map<&str, BuiltinFunction> = phf::phf_map! {
	...
	"terminal.input" => BuiltinFunction {
		evaluate_at_compile_time: |_context: &mut Context, _caller_scope_id: usize, _arguments: &[Expression]| {
			let mut line = String::new();
			std::io::stdin().read_line(&mut line)?;
			line = line.get(0..line.len() - 1).unwrap().to_owned();
			Ok(Expression::ObjectConstructor(ObjectConstructor::from_string(&line)))
		}
	}
};
...
pub fn call_builtin_at_compile_time(name: &str, context: &mut Context, caller_scope_id: usize, arguments: &[Expression]) -> anyhow::Result<Expression> {
	(BUILTINS.get(name).ok_or_else(|| anyhow::anyhow!("Unknown builtin"))?.evaluate_at_compile_time)(context, caller_scope_id, arguments)
}
