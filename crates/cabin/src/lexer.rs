...
pub enum TokenType {
	...
	/// The `tail` keyword token type, which is used to return a value from a scope.
	///
	/// A token created with this type will always have the value "its".
	///
	/// Like all keywords, this enum variant declaration *must* come before `Identifier`. If it doesn't, then `its` will be tokenized incorrectly as
	/// identifiers, which will cause issues when parsing. Please be careful when moving around this keyword or the `Identifier` token type!
	///
	/// TODO: We are considering renaming this keyword. Currently, it makes sense for small expressions like this:
	///
	/// ```cabin
	...
	/// ```
	///
	/// However, for larger blocks, this can be more confusing. No alternatives have been suggested yet.
	KeywordTail,
	...
	/// 	its "Portable networks graphics";
	/// } else {
	/// 	its "not a PNG!";
	/// };
	/// ```
...
impl TokenType {
	...
	fn pattern(&self) -> &'static regex_macro::Regex {
		match self {
			...
			Self::KeywordTail => regex_macro::regex!(r"^its\b"),
