...
impl TranspileToC for Program {
	...
	fn c_prelude(&self, context: &mut Context) -> anyhow::Result<String> {
		...
		let mut any = "union AnyPointer {\n".to_owned();
		for (group, _) in &context.groups {
			writeln!(any, "\t{group}* {group};").unwrap();
		}
		any.push_str("};\n");
		...
		prelude = format!(
			...
			unindent::unindent(
				...
				typedef union AnyPointer AnyPointer;
			"
			),
